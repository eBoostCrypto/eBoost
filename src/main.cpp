...
// Copyright (c) 2011-2013 eBoost Developers
...
uint256 hashGenesisBlock("0x21938e71a19f0053f1ea9d2c616feba7c767ade95a20946f26747ae96aaa572b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // eBoost: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "eBoost Signed Message:\n";
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal eboost miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    if (pindex->nHeight > COINFIX2_BLOCK)
    {
    	if(vtx[0].vout[0].scriptPubKey != GetFoundationScript()) {
    	   return error("ConnectBlock() : coinbase does not pay to the charity in the first output");
    	}
    }
    // Update block index on disk without changing it in memory.
    // The memory index structure will be changed after the db commits.
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "eBoost", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Worlds Tastiest Pizza Still Undiscovered";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1467694536;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1707074 ;
        ...
        assert(block.hashMerkleRoot == uint256("0xdabfed775df0856139f27b377eabf61b8bc5f72abc3c95540e5564ab3dbe6c60"));
...
unsigned char pchMessageStart[4] = { 0xfc, 0xd9, 0xb7, 0xdd }; // eBoost: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // eBoostd: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eBoostMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("eBoostMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadeBoostMiner(void* parg);

static bool fGenerateBitcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void static eBoostMiner(CWallet *pwallet)
{
    printf("eBoostMiner started\n");
    ...
    while (fGenerateBitcoins)
    {
        ...
        printf("Running eBoostMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadeBoostMiner(void* parg)
{
    ...
    try
    {
        ...
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadeBoostMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadeBoostMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadeBoostMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d eBoostMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadeBoostMiner, pwallet))
                printf("Error: CreateThread(ThreadeBoostMiner) failed\n");
            Sleep(10);
        }
    }
}
